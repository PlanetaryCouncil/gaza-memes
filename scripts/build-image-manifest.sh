...
build_from_folder_scan() {
  local folder="$1"

  find "$repo_root/$folder" -type f \
    | sed "s#^$repo_root/##" \
    | awk '
        index($0, "/.") == 0 &&
        $0 !~ /\/\.DS_Store$/ &&
        $0 !~ /^positive\/collages autogenerated\//
      '
}
...
build_manifest() {
  ...
  while IFS= read -r folder || [[ -n "$folder" ]]; do
    ...
    if [[ -f "$repo_root/$folder/readme.md" ]]; then
      ...
      folder_entries="$(build_from_root_readme "$folder")"
      if [[ -n "$folder_entries" ]]; then
        printf '%s\n' "$folder_entries" >> "$output_file"
      else
        build_from_folder_scan "$folder" >> "$output_file"
      fi
    fi
  done < "$folders_file"
...
build_manifest "$repo_root/data/images.txt" "$repo_root/data/folders_to_include.txt"
build_manifest "$repo_root/data/images-positive.txt" "$repo_root/data/folders_to_include_positive.txt"
